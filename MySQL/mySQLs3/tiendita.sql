...
VALUES ('Jugo Verduras', 12, curdate(), 8, 33, "V8");
select * from productos;
UPDATE productos SET marca = "Bubaloo" 
WHERE idProducto = 27;
...
UPDATE productos SET precio = REPLACE (precio, 5, 10); 
DELETE FROM categoria WHERE id = 13;
SET FOREIGN_key_checks = 0;
TRUNCATE TABLE categoria;
SET FOREIGN_key_checks = 1;
SELECT * FROM categoria WHERE id < 9;
SELECT idProducto FROM productos GROUP BY marca;
...
SELECT COUNT(existencia), nombre FROM productos GROUP BY nombre;
